...
import { captureMySelfie, getMyBusinessProfile, setMyCover, updateMyBusinessProfile, type SupabaseBusinessProfile } from "@/lib/supabase-business";
...
export type Profile = { name: string; category: string; location: string; followers: number; bio: string; selfieUri?: string; coverUri?: string; identityStatus: "pending" | "selfie_captured" | "approved" | "rejected"; phone?: string; contactEmail?: string; locked: boolean; };
...
type BusinessContextValue = {
  ...
  authenticated: boolean; authLoading: boolean; authError: string | null; loading: boolean; error: string | null; currentUserId?: string;
...
export function BusinessProvider({ children }: { children: ReactNode }) {
  ...
  const refreshAll = useCallback(async () => {
    if (!auth.isAuthenticated) { setProfile(emptyProfile); return; }
    try { setProfileLoading(true); setProfileError(null); const next = await getMyBusinessProfile(); setProfile(next ? mapProfile(next) : emptyProfile); }
  ...
  const value = useMemo<BusinessContextValue>(() => ({
    posts: [], profile, conversations: [], notifications: [], authenticated: auth.isAuthenticated, authLoading: auth.loading, authError: auth.error?.message ?? null, loading: profileLoading, error: profileError, currentUserId: auth.user?.id,
    refreshAll,
  ...
  }), [auth.error?.message, auth.isAuthenticated, auth.loading, auth.logout, auth.refresh, auth.user?.id, profile, profileError, profileLoading, refreshAll]);
