...
function SelfieFlow() {
  ...
  const mandatory = required === "1" || !["selfie_captured", "approved"].includes(profile.identityStatus);
  ...
  useEffect(() => {
    if (step !== "camera" || ready) return;
    const timeout = setTimeout(() => {
      setCameraError("La caméra n’a pas démarré. Vérifiez qu’aucune autre application n’utilise déjà l’appareil photo.");
      setStep("error");
    }, 10000);
  ...
  const startCamera = async () => {
    try {
      ...
      const result = currentPermission.granted ? currentPermission : await Camera.requestCameraPermissionsAsync();
      if (result.granted) {
        ...
        const device = Platform.OS === "android" ? "Android" : Platform.OS === "ios" ? "iPhone" : "navigateur";
        const hint = result.canAskAgain ? "Autorisez la caméra puis réessayez." : `Autorisez la caméra pour Business Ivoire dans les réglages de votre ${device}, puis réessayez.`;
        setCameraError(`L’accès à la caméra est refusé. ${hint}`);
        setStep("error");
      }
    } catch {
      setCameraAuthorized(false);
      setCameraError("La demande d’accès à la caméra a échoué avant le démarrage. Vérifiez l’autorisation caméra de Business Ivoire, puis réessayez.");
      setStep("error");
    } finally {
  ...
  const confirmSelfie = async () => {
    ...
    try {
      setBusy(true);
      console.info("[selfie] confirmation pressed");
      await setSelfie(selfieData);
      Alert.alert("Selfie enregistré", "Votre profil vendeur est maintenant vérifié. Vous pouvez utiliser Business Ivoire.", [{ text: "Continuer", onPress: () => router.replace(returnTo === "create" ? "/create" as never : "/" as never) }]);
    } catch (error) {
      console.error("[selfie] confirmation failed", error);
      const details = error instanceof Error && error.message ? ` ${error.message}` : "";
      Alert.alert("Enregistrement impossible", `Votre selfie n’a pas pu être sauvegardé.${details} Vérifiez votre connexion puis réessayez : votre photo reste disponible pour une nouvelle tentative.`);
    } finally {
      setBusy(false);
    }
  };

  if (step === "intro") return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.introScreen}>{!mandatory ? <TouchableOpacity accessibilityLabel="Fermer" onPress={() => router.back()} style={styles.closeButton}><MaterialIcons name="close" size={24} color="#102015" /></TouchableOpacity> : null}<View style={styles.introContent}><View style={styles.shield}><MaterialIcons name="verified-user" size={43} color="#D5A72C" /></View><Text style={styles.introTitle}>Vérifiez votre identité</Text><Text style={styles.introText}>Pour accéder à Business Ivoire, prenez maintenant un selfie en direct avec la caméra frontale. Il deviendra votre photo de profil vendeur.</Text><View style={styles.ruleCard}><MaterialIcons name="camera-front" size={23} color="#176B35" /><View style={styles.ruleCopy}><Text style={styles.ruleTitle}>Caméra frontale obligatoire</Text><Text style={styles.ruleText}>Le selfie est pris maintenant, en direct. La galerie n’est jamais accessible depuis cet écran.</Text></View></View><View style={styles.ruleCard}><MaterialIcons name="lock-outline" size={23} color="#176B35" /><View style={styles.ruleCopy}><Text style={styles.ruleTitle}>Accès bloqué jusqu’à la capture</Text><Text style={styles.ruleText}>Vous pourrez publier et envoyer des messages dès que votre selfie direct sera enregistré.</Text></View></View></View><View style={styles.introFooter}><TouchableOpacity disabled={busy || requestingCamera} onPress={startCamera} style={[styles.cameraButton, (busy || requestingCamera) && styles.disabled]}><MaterialIcons name="camera-front" size={21} color="#FFFFFF" /><Text style={styles.cameraButtonText}>{requestingCamera ? "Ouverture de la caméra…" : "Ouvrir la caméra frontale"}</Text></TouchableOpacity><Text style={styles.legalText}>En continuant, vous confirmez que cette photo est de vous.</Text></View></ScreenContainer>;

  if (step === "error" && cameraError) return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.deniedScreen}><MaterialIcons name="camera-alt" size={46} color="#D5A72C" /><Text style={styles.deniedTitle}>Caméra indisponible</Text><Text style={styles.deniedText}>{cameraError} Le selfie reste obligatoirement pris en direct par caméra frontale.</Text><TouchableOpacity style={styles.retryButton} onPress={startCamera}><Text style={styles.retryText}>Réessayer</Text></TouchableOpacity></ScreenContainer>;

  if (!cameraAuthorized) return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.deniedScreen}><MaterialIcons name="no-photography" size={46} color="#D5A72C" /><Text style={styles.deniedTitle}>Caméra requise</Text><Text style={styles.deniedText}>Le selfie ne peut pas venir de la galerie. Autorisez la caméra dans les réglages de votre téléphone, puis prenez votre visage en direct.</Text><TouchableOpacity style={styles.retryButton} onPress={startCamera}><Text style={styles.retryText}>Autoriser la caméra</Text></TouchableOpacity></ScreenContainer>;

  if (step === "review" && previewUri) return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.reviewScreen}><View style={styles.reviewHeader}><TouchableOpacity onPress={() => { setReady(false); setCameraError(undefined); setStep("camera"); }}><MaterialIcons name="arrow-back" size={25} color="#102015" /></TouchableOpacity><Text style={styles.reviewTitle}>Vérifiez votre selfie</Text><View style={styles.headerSpace} /></View><View style={styles.reviewContent}><Image source={{ uri: previewUri }} style={styles.preview} accessibilityLabel="Aperçu du selfie direct" /><Text style={styles.reviewText}>Assurez-vous que votre visage est net et bien éclairé. Cette photo prise en direct sera reliée à votre profil vendeur.</Text></View><View style={styles.reviewFooter}><TouchableOpacity disabled={busy} onPress={() => { setReady(false); setCameraError(undefined); setStep("camera"); }} style={styles.retakeButton}><MaterialIcons name="refresh" size={20} color="#176B35" /><Text style={styles.retakeText}>Reprendre</Text></TouchableOpacity><TouchableOpacity disabled={busy} onPress={() => void confirmSelfie()} style={[styles.confirmButton, busy && styles.disabled]}><MaterialIcons name="check" size={20} color="#FFFFFF" /><Text style={styles.confirmText}>{busy ? "Envoi sécurisé…" : "Confirmer"}</Text></TouchableOpacity></View></ScreenContainer>;

  if (step !== "camera") return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.deniedScreen}><Text style={styles.deniedText}>Préparation sécurisée de la caméra…</Text></ScreenContainer>;

  return <View style={styles.cameraScreen}><CameraView ref={cameraRef} facing="front" style={StyleSheet.absoluteFill} mirror onCameraReady={() => { setReady(true); setCameraError(undefined); }} onMountError={(error) => { setReady(false); setCameraError(error.message || "La caméra frontale n’est pas disponible sur cet appareil."); setStep("error"); }} /><View style={styles.cameraOverlay}><View style={styles.cameraHeader}>{!mandatory ? <TouchableOpacity onPress={() => router.back()} style={styles.cameraClose}><MaterialIcons name="close" size={25} color="#FFFFFF" /></TouchableOpacity> : <View style={styles.headerSpace} />}<Text style={styles.cameraTitle}>Cadrez votre vrai visage</Text><View style={styles.headerSpace} /></View><View style={styles.faceGuide}><View style={styles.guideOval} /><Text style={styles.guideText}>Placez votre visage dans le cercle</Text></View><View style={styles.cameraFooter}><Text style={styles.cameraHint}>Caméra frontale uniquement. Pas de filtre. Pas de galerie.</Text><TouchableOpacity accessibilityLabel="Prendre le selfie direct" disabled={!ready || busy} onPress={takeSelfie} style={[styles.shutter, (!ready || busy) && styles.shutterDisabled]}><View style={styles.shutterInner} /></TouchableOpacity></View></View></View>;
}

export default function SelfieScreen() {
  return <AuthGate accessOnly><SelfieFlow /></AuthGate>;
}
