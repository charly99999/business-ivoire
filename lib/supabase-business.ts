...
export async function fetchSupabaseConversations() {
  await currentUserId();
  const { data, error } = await supabase.rpc("get_my_conversation_inbox", { p_limit: 50 });
  if (error) throw error;
  return (data ?? []).map((conversation: { id: string; title?: string | null; kind: string; updated_at: string; member_name?: string | null; member_avatar_path?: string | null; last_body?: string | null; last_created_at?: string | null }) => {
    return {
      id: conversation.id,
      name: conversation.title || conversation.member_name || "Conversation Business Ivoire",
      preview: conversation.last_body || "Aucun message pour le moment.",
      time: conversation.last_created_at || conversation.updated_at,
      avatarUrl: conversation.member_avatar_path ? supabase.storage.from("profile-avatars").getPublicUrl(conversation.member_avatar_path).data.publicUrl : undefined,
    };
  });
}
